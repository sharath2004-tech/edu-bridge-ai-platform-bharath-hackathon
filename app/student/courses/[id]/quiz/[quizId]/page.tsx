...
export default function StudentQuizPage({ params }: { params: Promise<{ id: string; quizId: string }> }) {
  ...
  const fetchQuiz = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const course = data.data || data.courses?.[0]
        if (course?.quizzes?.[parseInt(resolvedParams.quizId)]) {
          setQuiz(course.quizzes[parseInt(resolvedParams.quizId)])
        }
      }
    } catch (error) {
    ...
    try {
      await fetch(`/api/student/quiz-response`, {
        ...
        body: JSON.stringify({
          ...
          quizIndex: parseInt(resolvedParams.quizId),
          answers,
          score: percentage,
        }),
      })
      ...
      <div className="space-y-6 animate-fadeIn max-w-3xl mx-auto">
        <Card className="p-8 text-center">
          ...
          <div className="flex gap-3 justify-center">
            <Link href={`/student/courses/${resolvedParams.id}`}>
  ...
  return (
    <div className="space-y-6 animate-fadeIn max-w-3xl mx-auto">
      <div>
        <Link href={`/student/courses/${resolvedParams.id}`} className="text-sm text-muted-foreground hover:underline mb-2 inline-block">
          ← Back to Course
        </Link>
